/* Inlined from assets/js/admin.js */
// ===== admin.html :: inline-script-01 =====
...
(function(){
  ...
  firebase.database = patchedDatabase;
                ...
                function sanitizeTenantId(value) {
                    return String(value || 'titan').trim().replace(/[^A-Za-z0-9_-]/g,'') || 'titan';
                }
                function personalAdminTenantId(userOrUid) {
                    const uid = typeof userOrUid === 'string' ? userOrUid : (userOrUid && userOrUid.uid);
                    const cleanUid = String(uid || '').trim().replace(/[^A-Za-z0-9_-]/g,'');
                    return cleanUid ? ('teacher_' + cleanUid) : 'titan';
                }
                function adminRoleAllowsAccess(role) {
                    return role === true || role === 'admin' || role === 'owner' || role === 'super_admin';
                }
                async function ensurePersonalAdminTenant(user, source) {
                    if (!user || !user.uid) throw new Error('لا يوجد مستخدم لتجهيز مساحة المدرس.');
                    const tenantId = personalAdminTenantId(user);
                    const profile = {
                        uid: user.uid,
                        email: user.email || '',
                        name: user.displayName || (user.email || '').split('@')[0] || 'مدرس',
                        role: 'owner',
                        isolated: true,
                        source: source || 'personal-admin-tenant',
                        tenantId: tenantId,
                        createdAt: Date.now(),
                        createdAtIso: new Date().toISOString()
                    };
                    const database = firebase.database();
                    await database.ref('tenants/' + tenantId + '/admins/' + user.uid).set(true);
                    await database.ref('user_tenants/' + user.uid + '/' + tenantId).set('owner');
                    await database.ref('tenants/' + tenantId + '/admin_profiles/' + user.uid).update(profile);
                    await database.ref('tenants/' + tenantId + '/settings/isolation').update({
                        mode: 'per_admin_private_tenant', ownerUid: user.uid, updatedAt: Date.now()
                    });
                    await database.ref('tenants/' + tenantId + '/audit_logs').push({
                        type: 'personal_tenant_ready', actor: user.uid, email: user.email || '', source: source || '', tenantId: tenantId,
                        createdAt: Date.now(), createdAtIso: new Date().toISOString()
                    }).catch(function(){});
                    try { await database.ref('public_tenants/' + tenantId).set({ownerUid:user.uid, public:true, createdAt:Date.now()}); } catch(e) {}
                    return tenantId;
                }
                function redirectToTenant(tenantId, message) {
                    const cleanTenant = sanitizeTenantId(tenantId);
                    localStorage.setItem('titanTenantId', cleanTenant);
                    const url = new URL(location.href);
                    url.searchParams.set('tenant', cleanTenant);
                    if (message) alert(message);
                    location.href = url.toString();
                }
                window.sanitizeTenantId = sanitizeTenantId;
                window.personalAdminTenantId = personalAdminTenantId;
                window.redirectToTenant = redirectToTenant;
                function currentTenantId() {
                    const t = (window.TitanSaaS && window.TitanSaaS.tenantId) || new URLSearchParams(location.search).get('tenant') || localStorage.getItem('titanTenantId') || 'titan'; return sanitizeTenantId(t);
                }
                function showAdminAccessHelp(user, extra) {
                    const uid = user && user.uid ? user.uid : 'UNKNOWN_UID';
                    const tenantId = currentTenantId();
                    const msg = 'لم يتم فتح لوحة الإدارة لهذا الحساب.\n\n' +
                        'UID: ' + uid + '\n' +
                        'Tenant الحالي: ' + tenantId + '\n\n' +
                        'تأكد من وجود واحد من هذه المسارات في Realtime Database:\n' +
                        'tenants/' + tenantId + '/admins/' + uid + ' = true\n' +
                        'أو super_admins/' + uid + ' = true\n' +
                        'أو user_tenants/' + uid + '/' + tenantId + ' = admin\n\n' +
                        (extra || '');
                    console.warn(msg);
                    const dbg = document.getElementById('admin-debug-box');
                ...
                async function resolveAdminAccess(user) {
                    ...
                    const uid = user.uid;
                    const tenantId = currentTenantId();
                    const personalTenant = personalAdminTenantId(user);

                    // 1) Super admin works from any tenant and is the only account that can intentionally see shared tenants.
                    try {
                        const superSnap = await firebase.database().ref('super_admins/' + uid).once('value');
                        if (superSnap.exists() && superSnap.val() === true) return { ok:true, role:'super_admin', tenant:tenantId };
                    } catch (e) { console.warn('super_admins check failed', e); }

                    // 2) If the account already has its private teacher tenant, always use it.
                    try {
                        const utSnap = await firebase.database().ref('user_tenants/' + uid).once('value');
                        const tenants = utSnap.val() || {};
                        if (adminRoleAllowsAccess(tenants[personalTenant])) {
                            if (tenantId !== personalTenant) return { ok:false, reason:'wrong_tenant', correctTenant:personalTenant };
                            return { ok:true, role:String(tenants[personalTenant]), tenant:personalTenant };
                        }
                        const anyAdminTenant = Object.keys(tenants).find(t => adminRoleAllowsAccess(tenants[t]));
                        if (anyAdminTenant) {
                            // Legacy/shared admin account: create a private space and move this login to it.
                            const readyTenant = await ensurePersonalAdminTenant(user, 'isolated_from_user_tenants_' + anyAdminTenant);
                            return { ok:false, reason:'wrong_tenant', correctTenant:readyTenant };
                        }
                    } catch (e) { console.warn('user_tenants check failed', e); }

                    // 3) Legacy check: old builds stored many public admin accounts in tenants/titan/admins.
                    // If found, do not open the shared data; create and redirect to the teacher's private tenant.
                    try {
                        const adminSnap = await firebase.database().ref('tenants/' + tenantId + '/admins/' + uid).once('value');
                        if (adminSnap.exists() && adminSnap.val() === true) {
                            if (tenantId !== personalTenant) {
                                const readyTenant = await ensurePersonalAdminTenant(user, 'isolated_from_legacy_tenant_' + tenantId);
                                return { ok:false, reason:'wrong_tenant', correctTenant:readyTenant };
                            }
                            return { ok:true, role:'owner', tenant:personalTenant };
                        }
                    } catch (e) { console.warn('tenant admin check failed', e); return { ok:false, reason:'permission_or_rules', error:e }; }
                ...
                async function userIsAdmin(user) {
                    ...
                    if (access.reason === 'wrong_tenant' && access.correctTenant) {
                        redirectToTenant(access.correctTenant, 'تم تجهيز مساحة خاصة لهذا الحساب. سيتم فتح لوحة الإدارة على tenant الخاص: ' + access.correctTenant);
                        return false;
                    }
                    return false;
                }
                ...
                window.adminLoginFromFirebase = async function(email, password) {
                    ...
                    try {
                        ...
                        const access = await resolveAdminAccess(cred.user);
                        if (access.ok) {
                            openAdminApp(cred.user);
                            return;
                        }
                        if (access.reason === 'wrong_tenant' && access.correctTenant) {
                            redirectToTenant(access.correctTenant, 'تم تسجيل الدخول. سيتم التحويل إلى مساحة الحساب الخاصة: ' + access.correctTenant);
                            return;
                        }
                        // لا نعمل signOut هنا؛ لأن Firebase Auth مشترك مع صفحات الطالب وولي الأمر.
                        showAdminAccessHelp(cred.user, access.error ? ('تفاصيل الخطأ: ' + (access.error.code || access.error.message || access.error)) : '');
                    } catch (err) {
                        console.error(err);
                        alert('تعذر تسجيل الدخول. الكود التقني: ' + (err.code || err.message || err) + (String(err.code||'').includes('unauthorized-domain') ? '\n\nالحل: أضف الدومين الحالي في Firebase Authentication > Settings > Authorized domains. لو Live Server أضف 127.0.0.1 أو استخدم localhost.' : ''));
                    }
                };
...
// ===== admin.html :: titan-admin-share-note-v6 =====
...
// ===== admin.html :: titan-file-origin-note-v7 =====
window.addEventListener('DOMContentLoaded', function(){
  try {
    if(location.protocol === 'file:') {
      var box=document.createElement('div');
...
// ===== admin.html :: titan-admin-v8-features =====
(function(){
  const $ = (id) => document.getElementById(id);
  ...
  function openPrintWindow(html){
    ...
    const doc = [
      ...
      '<style>',
      'body{font-family:Cairo,Arial,sans-serif;background:#fff;color:#0f172a;margin:0;padding:24px;line-height:1.8}',
      '.report,.certificate,.card{max-width:980px;margin:auto;border:1px solid #e2e8f0;border-radius:18px;padding:22px;box-shadow:0 10px 30px rgba(15,23,42,.08)}',
      'h1,h2,h3{color:#2563eb;margin:0 0 12px}',
      'table{width:100%;border-collapse:collapse;margin-top:14px}',
      'td,th{border:1px solid #e2e8f0;padding:10px;text-align:center}',
      '@media print{body{padding:0}.report,.certificate,.card{box-shadow:none;border:0}}',
      '</style>',
      '</head>',
      '<body>',
      safeHtml,
...
// ===== admin.html :: titan-admin-v9-protection =====
(function(){
  const $=(id)=>document.getElementById(id);
...
// ===== admin.html :: titan-admin-v10-mega-notifications =====
(function(){
  const $=id=>document.getElementById(id);
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const nowIso=()=>new Date().toISOString();
  ...
  async function addMegaRecord(){
    ...
    await db().ref('mega_records').push({type,title:'سجل تجريبي',body:'تمت إضافته لاختبار البحث والتصدير واستيعاب السجلات الكبيرة.',status:'active',createdAt:nowIso(),when:nowAr()});
...
// ===== admin.html :: titan-v14-saas-admin-js =====
(function(){
  const $=id=>document.getElementById(id);
  const esc=v=>String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const db=()=>window.firebase&&firebase.database?firebase.database():null;
  ...
  async function createAttendanceQR(){
    ...
    const url=location.origin+location.pathname.replace(/[^\/]+$/,'')+'student.html?tenant='+encodeURIComponent(tenant())+'&attendance='+encodeURIComponent(code);
...
// ===== admin.html :: titan-v14-auth-fix-admin-v2-js =====
(function(){
  ...
  async function createStudentAccountV2(){
    try{
      ...
      const email = window.makeStudentEmail ? window.makeStudentEmail(code) : (String(code).trim().toLowerCase().replace(/[^a-z0-9._-]/g,'') + '.' + String((window.TitanSaaS&&TitanSaaS.tenantId)||'titan').trim().toLowerCase().replace(/[^a-z0-9._-]/g,'') + '@students.titan-academy.app');
...
// ===== admin.html :: titan-v15-customization-admin-js =====
(function(){
  const DEFAULTS = {
    branding:{
      name:'أكاديمية Programmers',
      teacher:'',
      logo:'',
      favicon:'',
      whatsapp:'',
...
// ===== admin.html :: titan-v15-live-server-diagnostic-js =====
(function(){
  window.addEventListener('DOMContentLoaded', function(){
    try{
      if(location.protocol === 'file:'){
...
(function(){
  ...
  <section id="sec-v16-features" style="display:none;">
    <div class="v16-hero"><h3><i class="fas fa-rocket"></i> مركز المميزات المتقدمة</h3><p>إدارة الاشتراكات، المسارات، تحديات البرمجة، المساعد الذكي، التحليلات، الواتساب، المشاريع، والتذاكر المتقدمة.</p><div id="v16-admin-health" class="v16-status">جاري تشغيل الإضافات...</div></div>
    <div class="v16-tabs">
  ...
  function install(){
    ...
    if(nav&&!document.querySelector('[data-v16-admin-nav]')){ const btn=document.createElement('button'); btn.className='btn btn-outline'; btn.setAttribute('data-v16-admin-nav','1'); btn.setAttribute('onclick',"showSec('v16-features')"); btn.innerHTML='<i class="fas fa-rocket"></i> المميزات المتقدمة'; nav.insertBefore(btn, nav.lastElementChild); }
